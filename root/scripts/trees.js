...
button.addEventListener('click', function () {
   var c1=scaleIt(treeImages[treeIndex],0.50);
   canvas.width=c1.width/2;
   canvas.height=c1.height/2;
   ctx.drawImage(c1,0,0,250,167);

   // ctx.drawImage(
   //    treeImages[treeIndex % treeImages.length],
   //    xCoordinate[treeIndex % xCoordinate.length],
   //    yCoordinate[treeIndex % yCoordinate.length],
   //    treeImages[treeIndex % treeImages.length].width / resizingFactor,
   //    treeImages[treeIndex % treeImages.length].height / resizingFactor,
   // );
   treeIndex++;
